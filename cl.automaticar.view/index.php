<!DOCTYPE HTML>
<!--
	Transitive by TEMPLATED
	templated.co @templatedco
	Released for free under the Creative Commons Attribution 3.0 license (templated.co/license)
-->
<html>
	<head>
		<title>AutomatiCar</title>
		<meta charset="utf-8" />
		<meta name="viewport" content="width=device-width, initial-scale=1" />
		<link rel="stylesheet" href="../assets/css/main.css" />
	</head>
	<body>

			<header id="header" class="alt">
				<div class="logo"><a href="index.html">AutomatiCar </a></div>
				<a href="#menu" class="toggle"><span>Menú</span></a>
			</header>
            
			<nav id="menu">
				<ul class="links">
					<li><a href="index.php">Inicio</a></li>
					<li><a href="Login.php">Iniciar sesión</a></li>
                                        <li><a href="CrearCuenta.php">¿No estás registrado? registrate aquí</a></li>
                                        <li><a href="fullcalendar.php">Servicios</a></li>
				</ul>
			</nav>

			<section id="banner" data-video="images/banner">
				<div class="inner">
					<h1>AutomatiCar</h1>
					<p>Nuestro software te brindará el control y el manejo indispensable<br/>
					que necesitas para tu<a href="#"> Taller mecánico</a></p>
					<a href="#one" class="button special scrolly">Iniciar sesión</a>
				</div>
			</section>

		<!-- One -->
			<section id="one" class="wrapper style2">
				<div class="inner">
					<div>
						<div class="box">
							<div class="image fit">
								<img src="images/pic01.jpg" alt="" />
							</div>
							<div class="content">
								<header class="align-center">
									<h2>h2 en Linea 56 de index.html</h2>
									<p>Parrafo en Linea 57 de index.html</p>
								</header>
								<hr />
								<p>Parrafo en Linea 60 de index.html.</p>
								<p>Parrafo en Linea 61 de index.html</p>
							</div>
						</div>
					</div>
				</div>
			</section>


			<section id="two" class="wrapper style3">
				<div class="inner">
					<div id="flexgrid">
						<div>
							<header>
								<h3>h3 en Linea 74 de index.html</h3>
							</header>
							<p>Parrafo en Linea 76 de index.html</p>
							<ul class="actions">
								<li><a href="#" class="button alt">Learn More</a></li>
							</ul>
						</div>
						<div>
							<header>
								<h3>h3 en Linea 83 de index.html</h3>
							</header>
							<p>Parrafo en Linea 85 de index.html </p>
							<ul class="actions">
								<li><a href="#" class="button alt">Learn More</a></li>
							</ul>
						</div>
						<div>
							<header>
								<h3>h3 en Linea 92 de index.html</h3>
							</header>
							<p>Parrafo en Linea 94 de index.html</p>
							<ul class="actions">
								<li><a href="#" class="button alt">Learn More</a></li>
							</ul>
						</div>
					</div>
				</div>
			</section>


			<section id="three" class="wrapper style2">
				<div class="inner">
					<div class="grid-style">

						<div>
							<div class="box">
								<div class="image fit">
									<img src="images/pic02.jpg" alt="" />
								</div>
								<div class="content">
									<header class="align-center">
										<h2>h2 en Linea 115 de index.html</h2>
										<p>Parrafo en Linea 116 de index.html</p>
									</header>
									<hr />
									<p> Parrafo en Linea 119 de index.html</p>
								</div>
							</div>
						</div>

						<div>
							<div class="box">
								<div class="image fit">
									<img src="images/pic03.jpg" alt="" />
								</div>
								<div class="content">
									<header class="align-center">
										<h2>h2 en Linea 131 de index.html</h2>
										<p>Parrafo en Linea 132 de index.html</p>
									</header>
									<hr />
									<p> Parrafo en Linea 135 de index.html</p>
								</div>
							</div>
						</div>

					</div>
				</div>
			</section>


			<section id="four" class="wrapper style3">
				<div class="inner">

					<header class="align-center">
						<h2>h2 en Linea 142 de index.html</h2>
						<p>Parrafo en Linea 143 de index.html</p>
					</header>

				</div>
			</section>


			<footer id="footer" class="wrapper">
				<div class="inner">
					<section>
						<div class="box">
							<div class="content">
								<h2 class="align-center">Agendar Hora</h2>
								<hr />
								<form action="#" method="post">
									<div class="field half first">
										<label for="name">Número de patente</label>
										<input name="name" id="name" type="text" placeholder="Patente">
									</div>
									<div class="field half">
										<label for="email">E-mail</label>
										<input name="email" id="email" type="email" placeholder="Email">
									</div>
									<div class="field">
										<label for="dept">Department</label>
										<div class="select-wrapper">
											<select name="dept" id="dept">
												<option value="">- linea 170 -</option>
												<option value="1">linea 171</option>
												<option value="1">linea 172</option>
												<option value="1">linea 173</option>
												<option value="1">linea 174</option>
											</select>
										</div>
									</div>
									<div class="field">
										<label for="message">Message</label>
										<textarea name="message" id="message" rows="6" placeholder="Message"></textarea>
									</div>
									<ul class="actions align-center">
										<li><input value="Send Message" class="button special" type="submit"></li>
									</ul>
								</form>
							</div>
						</div>
					</section>
					<div class="copyright">
						&copy; linea 190 de index: <a href="https://templated.co/">TEMPLATED</a>. Images <a href="https://unsplash.com/">Unsplash</a>. Video <a href="http://coverr.co/">Coverr</a>.
					</div>
				</div>
			</footer>


			<script src="../assets/js/jquery.min.js"></script>
			<script src="../assets/js/jquery.scrolly.min.js"></script>
			<script src="../assets/js/jquery.scrollex.min.js"></script>
			<script src="../assets/js/skel.min.js"></script>
			<script src="../assets/js/util.js"></script>
			<script src="../assets/js/main.js"></script>

	</body>
</html>
